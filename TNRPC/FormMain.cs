                                    ...
                                    using (MySqlCommand cmd = new MySqlCommand("", conn)) {
                                        for (int j = 0; j <= 3; j++) {
                                            if (jfpg[j] > 0 && jfpg[j] < 1000000.0) {
                                                ...
                                                cmd.ExecuteNonQuery();
                                                SetText("label" + equipmentID + labelCode[j], jfpg[j].ToString("0.0"));
                                            } else {
            ...
            while (true) {
                try {
                    foreach (string plc in plcs) {
                        ...
                        double douWendu = Convert.ToDouble(strWendu) / 10.0;
                        double douShidu = Convert.ToDouble(strShidu) / 10.0;
